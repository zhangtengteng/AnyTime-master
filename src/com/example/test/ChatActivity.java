...
public class ChatActivity extends BaseActivity implements View.OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// get argument
		final String conversationId = getIntent().getStringExtra(
				EXTRA_CONVERSATION_ID);
		Log.d(TAG, "浼氳瘽 id: " + conversationId);

		// register callback
		handler = new ChatHandler(adapter);
		MessageHandler.setActivityMessageHandler(handler);

		conversation = AnyTimeApplication.getIMClient()
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (scrollState == SCROLL_STATE_IDLE) {
			if (view.getChildCount() > 0) {
				...
				if (first != null && view.getFirstVisiblePosition() == 0
						&& first.getTop() == 0) {
					loadOldMessages();
				}
			}
	...
	public class ChatHandler extends AVIMTypedMessageHandler<AVIMTypedMessage> {
		...
		public void onMessage(AVIMTypedMessage message,
			...
			if (conversation.getConversationId().equals(
					ChatActivity.this.conversation.getConversationId())) {
